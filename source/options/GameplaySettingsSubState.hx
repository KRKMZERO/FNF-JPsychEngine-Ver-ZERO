...
class GameplaySettingsSubState extends BaseOptionsMenu
{
	public function new()
	{
		...
		//追加部分
		var option:Option = new Option('Dialogue Language',
			"ダイアログの言語が選べます(MODによっては非対応)\nDialogue language can be selected (not supported by some mods)",
			'dialoguelanguage',//変数獲得のために必須
			'string',
			'Japanese',
			['Japanese','English']);
		addOption(option);
		...
		option.minValue = 21;
		option.maxValue = 180;
		option.changeValue = 2;
		addOption(option);

		var option:Option = new Option('Bad Hit Window',
			'Bad判定の最大時間をミリ秒単位で変更します。\n（デフォルト:135ms，短押しで3ms調整します。）',
			'badWindow',
			'int',
			135);
		option.displayFormat = '%vms';
